...
[ -z "$current_time" ] && current_time="未获取到"
ui_print "－ ●当前时间: $current_time ●"

device_name=$(getprop ro.vivo.market.name)
[ -z "$device_name" ] && device_name="未获取到"
ui_print "－ ●机型名: $device_name ●"

system_version=$(getprop ro.build.version.bbk)
[ -z "$system_version" ] && system_version="未获取到"
ui_print "－ ●系统版本: $system_version ●"

kernel_ver=$(cat /proc/version | awk '{print $3}')
[ -z "$kernel_ver" ] && kernel_ver="未获取到"
ui_print "－ ●内核版本: $kernel_ver ●"

root_env=$(get_root_type)
[ -z "$root_env" ] && root_env="未获取到"
ui_print "－ ●ROOT环境: $root_env ●"

phone=$(get_vivo_phone)
[ -z "$phone" ] && phone=$(get_oplus_phone)
[ -z "$phone" ] && phone="未获取到"
ui_print "－ ●您的手机号: $phone ●"
...
[ -z "$flashed_kernel" ] && flashed_kernel="未获取到"
